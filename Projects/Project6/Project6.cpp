...
int minRec2(int x[], int n) {
    ...
    int min = minRec2(x, n/2);
    int* half = x+(n/2);
    int min2 = minRec2(half, n-(n/2));
...
int solveMazeRec(int row, int col) {
    ...
    // Init values of neighbors
    int right;
    int left;
    int up;
    int down;

    // Check in bounds and populate value of neighbors
    if(col + 1 <= MATRIX_SIZE - 1)
         right = maze[row][col+1];
    if(col - 1 >= 0)
         left = maze[row][col-1];
    if(row + 1 <= MATRIX_SIZE -1)
         up = maze[row+1][col];
    if(row - 1 >= 0)
         down = maze[row-1][col];
    ...
    // No paths to goal returned, un-visit current square and propagate back up
    maze[row][col] = 0;
    return false;
